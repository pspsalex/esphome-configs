...
async def to_code(config):
    ...
    if CONF_D2_PIN not in config:
        depth=1
    else:
        depth=4
    esp32.add_idf_sdkconfig_option(
        f"CONFIG_ESP_HOSTED_SDIO_{depth}_BIT_BUS",
        True
    )
    esp32.add_idf_sdkconfig_option(
        f"CONFIG_ESP_HOSTED_PRIV_SDIO_PIN_D1_{depth}BIT_BUS_SLOT_{config[CONF_SLOT]}",
        config[CONF_D1_PIN],
    )
    ...
    if framework_ver >= cv.Version(5, 5, 0):
        ...
        esp32.add_idf_component(name="espressif/esp_hosted", ref="2.11.3")
    else:
